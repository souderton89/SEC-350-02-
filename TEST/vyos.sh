...
zone_add_binding_safe() {
  ...
  if [ "$to" = "$from" ]; then
    tprint ""; tprint "ERROR: TO and FROM cannot be the same zone."; pause; return 0
  fi

  if binding_exists "$to" "$from"; then
    existing_rs="$(binding_get_ruleset "$to" "$from")"
    tprint ""; tprint "ERROR: Binding already exists: $to <- $from  =  ${existing_rs:-UNKNOWN}"
    tprint "Use Update/Delete in Zone Bindings menu."; pause; return 0
  fi
...
zone_update_binding_existing() {
  ...
  if ! binding_exists "$to" "$from"; then
    tprint ""; tprint "ERROR: No existing binding for: $to <- $from"
    tprint "Use ADD if you want to create it."; pause; return 0
  fi

  existing_rs="$(binding_get_ruleset "$to" "$from")"
  tprint ""; tprint "Current ruleset for $to <- $from : ${existing_rs:-UNKNOWN}"
...
zone_delete_binding_existing() {
  ...
  if ! binding_exists "$to" "$from"; then
    tprint ""; tprint "ERROR: No existing binding for: $to <- $from"; pause; return 0
  fi
...
# ============================================================
# Why this matters:
#   Without 'default-log' on a firewall policy, VyOS silently
#   drops packets that hit the default-action with zero visibility
#   in syslog/journald. This is the gap present in EDGE01 configs
#   where DMZ-to-LAN, LAN-to-DMZ, and LAN-to-WAN were all missing
#   default-log — dropped packets left no audit trail.
#
# Functions:
#   fw_get_all_policy_names  — returns every ipv4 named policy
#   fw_policy_has_default_log — checks if default-log is set
#   fw_policy_default_action  — returns drop/accept/reject or -
#   fw_zone_logging_audit     — prints colour-coded table,
#                               offers to fix all missing at once
# ============================================================

# FIX-QC1: fw_get_all_policy_names removed — was a pointless wrapper
# around scan_firewall_rulesets. Callers now use scan_firewall_rulesets
...
# _fw_divider: print a 60-char dashed line compatible with vbash (no brace expansion).
# FIX-QC3: replaced printf '%0.s-' {1..60} — brace expansion is a bashism
# that is not guaranteed in vbash. awk is always available on VyOS.
_fw_divider() { awk 'BEGIN{for(i=0;i<60;i++) printf "-"; print ""}' >"$TTY"; }

# Audit table: show every policy with default-action + logging status.
# Offer to enable default-log on all missing policies in one commit.
# FIX-QC1: load_array now calls scan_firewall_rulesets directly (wrapper removed).
# FIX-QC2: log_label / log_display dead variables removed; tprintf uses
#           the log_state string directly.
# FIX-QC1 (recursion): post-apply re-audit is now a second pass of the
#           same loop inside this function — no recursive call.
fw_zone_logging_audit() {
  local policies=() missing=() policy da da_display log_state yn p
  local col_pol=32 col_act=22

  # ── First pass: scan and display ─────────────────────────────
  while true; do
    tprint ""
    tprint "====== Zone Policy Logging Audit ======"
    tprint ""
    tprint "Scanning all ipv4 firewall policies..."
    tprint ""

    policies=()
    load_array policies scan_firewall_rulesets

    if [ "${#policies[@]}" -eq 0 ]; then
      tprint "(no ipv4 firewall policies found)"
      pause; return 0
    fi
    ...
    for policy in "${policies[@]}"; do
      ...
      # Flag accept default-action as a warning — should almost always be drop
      case "$da" in
        drop)   da_display="drop" ;;
        accept) da_display="accept  <-- WARNING" ;;
        *)      da_display="$da" ;;
      esac

      if fw_policy_has_default_log "$policy"; then
        log_state="[OK]   dropped pkts logged"
      else
        log_state="[MISS] dropped pkts NOT logged"
        missing+=("$policy")
      fi
    ...
    if [ "${#missing[@]}" -eq 0 ]; then
      tprint ""
      tprint "  RESULT: All ${#policies[@]} policies have default-log enabled."
      tprint "          Your audit trail is complete."
      tprint ""
      pause; return 0
    fi
    ...
    # ── Some missing ─────────────────────────────────────────────
    tprint ""
    tprint "  RESULT: ${#missing[@]} of ${#policies[@]} policies are MISSING default-log:"
    for p in "${missing[@]}"; do tprintf "    - %s\n" "$p"; done
    tprint ""
    tprint "  These policies drop packets with NO syslog entry."
    tprint ""

    yn="$(choose_yes_no "Enable default-log on all ${#missing[@]} missing policies now?" "y" || echo "n")"
    if [ "$yn" != "y" ]; then
      tprint ""
      tprint "Not applied. Commands to run manually in configure mode:"
      tprint "--------------------------------------------------------"
      for p in "${missing[@]}"; do
    ...
    tprint ""
    tprint "Re-running audit to verify all policies are now covered..."
  done
}
...
firewall_menu() {
  ...
  while true; do
    ...
    tprint "6) Zone logging audit  (check / enable default-log on all policies)"
    tprint "7) Back"
    local c; tread c "Select: " || continue
    case "$c" in
      1) fw_list_ruleset ;;
...
#
# Changes the password of an existing VyOS login user.
# Uses the same plaintext-password mechanism as user_add_menu —
# VyOS hashes it to SHA-512 on commit, so it never persists as
# plaintext after save.
#
# Safety rules (matching the rest of this script):
#   - User must exist in the config tree (scan_login_users)
#   - Cannot change password for the currently logged-in user
#     without an explicit override confirm (different from remove
#     which blocks it outright — changing your own pw is valid)
#   - Two-prompt confirmation with tread_secret (hidden input)
#   - Passwords must match before cfg_begin is opened
#   - cfg_begin / cfg_apply pattern — same as all other ops
# ──────────────────────────────────────────────────────────────
user_change_password_menu() {
  ...
  tprint ""
  tprint "=== Change User Password ==="
  tprint "Existing VyOS users:"
  tprint ""

  select_from_list "Select user to change password" "${users[@]}" || return 0
  target="$SELECTED"

  # Warn (not block) if changing own password — valid operation
  if [ -n "$current" ] && [ "$target" = "$current" ]; then
    tprint ""
    tprint "NOTE: You are changing your OWN password ($current)."
    tprint "      Make sure you remember it — you will need it for future logins."
    yn="$(choose_yes_no "Continue?" "y" || echo "n")"
    [ "$yn" != "y" ] && { tprint "Canceled."; pause; return 0; }
  fi

  tprint ""
  tprint "Setting new password for: $target"
  tprint "(Input is hidden. VyOS will hash to SHA-512 on commit.)"
  tprint ""

  # Two-prompt loop until passwords match
  # FIX-QC5: pw1 and pw2 explicitly cleared at the top of each iteration
  #           so a failed tread_secret on pw2 never leaks a stale value
  #           into the next comparison.
  # FIX-QC6: tread_secret pw2 now has || return 0 so a TTY timeout on
  #           the confirmation prompt exits cleanly instead of falling
  #           through and comparing blank pw2 against pw1.
  while true; do
    pw1="" pw2=""
    ...
    tprint ""
    tprint "ERROR: Passwords do not match. Try again."
    tprint ""
  done

  tprint ""
  tprint "SUMMARY: change password for '$target'"
  tprint "         (plaintext sent to config mode, hashed to SHA-512 on commit)"
  tprint ""
  yn="$(choose_yes_no "Proceed?" "y" || echo "n")"
  [ "$yn" != "y" ] && { tprint "Canceled."; pause; return 0; }

  cfg_begin || return 0
  cfg_set system login user "$target" authentication plaintext-password "$pw1"
  cfg_apply
}
...
users_menu() {
  ...
  while true; do
    ...
    tprint "3) Change user password"
    tprint "4) Back"
    local c; tread c "Select: " || continue
    case "$c" in
      1) user_add_menu ;;
      2) user_remove_menu ;;
      3) user_change_password_menu ;;
      4) return 0 ;;
      *) tprint "Invalid." ;;
    esac
  done
...
dns_list_name_servers() {
  tprint ""; tprint "Current name-servers:"
  tprint "--------------------------------------------------------"
  local ns=(); load_array ns scan_dns_name_servers
  if [ "${#ns[@]}" -eq 0 ]; then tprint "  (none configured)"
  else local n; for n in "${ns[@]}"; do tprint "  $n"; done; fi
  tprint "--------------------------------------------------------"
  tprint ""; tprint "Raw config lines:"
  tprint "--------------------------------------------------------"
...
dns_add_name_server_safe() {
  ...
  [ -z "$ip" ] && return 0
  is_valid_ipv4 "$ip" || { tprint "ERROR: Must be a valid IPv4 address."; pause; return 0; }
  is_in_list "$ip" "${current[@]}" && { tprint "ERROR: $ip already configured."; pause; return 0; }
  tprint ""; tprint "SUMMARY: set system name-server $ip"
  yn="$(choose_yes_no "Proceed?" "y" || echo "n")"
  [ "$yn" != "y" ] && { tprint "Canceled."; pause; return 0; }
...
dns_delete_name_server_existing() {
  ...
  tprint ""; tprint "Deleting: system name-server $target"
  yn="$(choose_yes_no "Proceed?" "n" || echo "n")"
  [ "$yn" != "y" ] && { tprint "Canceled."; pause; return 0; }
...
dns_add_domain_forwarding_safe() {
  ...
  [ -z "$domain" ] && return 0
  is_valid_hostname "$domain" || { tprint "ERROR: Invalid domain name."; pause; return 0; }
  is_in_list "$domain" "${current[@]}" && { tprint "ERROR: Domain '$domain' already has a forwarding entry."; pause; return 0; }
  server="$(ask "Server IP to forward $domain queries to" "")"
  [ -z "$server" ] && return 0
  is_valid_ipv4 "$server" || { tprint "ERROR: Must be a valid IPv4 address."; pause; return 0; }
  tprint ""; tprint "SUMMARY: set service dns forwarding domain $domain name-server $server"
  yn="$(choose_yes_no "Proceed?" "y" || echo "n")"
  [ "$yn" != "y" ] && { tprint "Canceled."; pause; return 0; }
...
dns_delete_domain_forwarding_existing() {
  ...
  select_from_list "Select domain forwarding entry to DELETE" "${current[@]}" || return 0; target="$SELECTED"
  tprint ""; tprint "Deleting: dns forwarding domain $target"
  tprint "--------------------------------------------------------"
  { grep_cfg "set service dns forwarding domain $target "; grep_cfg "set service dns forwarding domain '$target' "; } >"$TTY" 2>/dev/null || true
  tprint "--------------------------------------------------------"
  yn="$(choose_yes_no "Proceed?" "n" || echo "n")"
  [ "$yn" != "y" ] && { tprint "Canceled."; pause; return 0; }
