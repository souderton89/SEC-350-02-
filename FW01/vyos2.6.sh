...
# Dynamic CRUD menu: Interfaces + Firewall + NAT + System + DNS + RIP
#
# EFFICIENCY (v2 rewrite):
# - CONFIG CACHE: get_cfg_cmds output is captured ONCE per menu action into
#   _CFG_CACHE and _CFG_CACHE_VALID. All scan functions read from the cache
#   with grep_cfg(). Cache is invalidated after every commit so the next
#   menu render always sees fresh state.
# - MERGED INTERFACE MENU: one unified menu covering eth/bond/VLAN/loopback.
#   The old split simple+expanded structure is gone.
# - RELEVANT SUMMARIES: each submenu shows only what it needs, not the full
#   global summary every loop iteration.
# - REDUCED PAUSES: list/show operations that already display output inline
#   no longer double-pause. The pause() call is only used where the screen
#   would clear before the user reads the output.
#
# SAFETY (carried forward):
# - ADD never overwrites existing items.
# - All user input is validated before reaching cfg_set/cfg_delete.
# - grep -F used throughout (no regex injection from user input).
# - reject_if_unsafe_commandline guards raw mode.
#
# AUDIT FIXES (all retained from v1):
# FIX-1  through FIX-15 — see original comments inline.
...
#
# All scan functions call grep_cfg() instead of piping from
# a fresh run show configuration commands each time.
#
# Cache lifecycle:
#   cfg_cache_refresh  — called once at each submenu entry (via
#                        warn_if_no_access) and after every commit.
#   cfg_cache_invalidate — called by cfg_apply after commit so
#                          the next render gets fresh data.
#   grep_cfg           — all scan functions use this instead of
#                        get_cfg_cmds | grep -F "..."
# ============================================================
_CFG_CACHE=""
_CFG_CACHE_VALID=0
...
cfg_cache_refresh() {
  # Fetch config once and store. Returns 1 if access fails.
  local out
...
# All scan functions use this instead of get_cfg_cmds | grep -F
grep_cfg() {
  # grep_cfg "pattern" — searches the cache; refreshes if stale
...
# Legacy wrapper kept for check_config_access and raw show needs
get_cfg_cmds() {
  if [ "$_CFG_CACHE_VALID" -eq 0 ]; then
    cfg_cache_refresh || return 1
  fi
...
# FIX-5: port range extraction with comments
is_valid_port_or_range() {
  local p="$1"
  echo "$p" | grep -Eq '^[0-9]{1,5}(-[0-9]{1,5})?$' || return 1
  local a="${p%%-*}"   # left side of range (or whole value if no dash)
  [ "$a" -ge 1 ] 2>/dev/null || return 1
  [ "$a" -le 65535 ] 2>/dev/null || return 1
  if echo "$p" | grep -q -- '-'; then
    local b="${p#*-}"  # right side of range
...
cfg_save() {
  ...
  tprint "Run: ls -l /opt/vyatta/sbin | grep -i save"
  return 1
}
...
# FIX-11: clean session close on early-exit error paths
...
# ACCESS GUARDS
# ============================================================
# FIX-14: per-submenu access check
warn_if_no_access() {
  # Refresh cache; if it fails, warn and return 1
...
# FIX-12: all output to $TTY; only echo final value to stdout for $() capture
choose_yes_no() {
...
cfg_apply() {
  ...
  # Always invalidate after any cfg_apply attempt so next render is fresh
  cfg_cache_invalidate
  pause
  return 0
}
...
# ============================================================
# SCAN FUNCTIONS
# All use grep_cfg() — reads from cache, not a new CLI call
# ============================================================
...
scan_vlan_ifaces()     { grep_cfg " vif " | grep -F "set interfaces ethernet " | awk '{print $4 "." $6}' | sort -u | while read -r x; do strip_quotes "$x"; done; }
scan_loopback_ifaces() { grep_cfg "set interfaces loopback " | awk '{print $4}' | sort -u | while read -r x; do strip_quotes "$x"; done; }
scan_all_ifaces() {
  # One grep_cfg pass per type — still uses cache, just 4 greps instead of 4 subshell CLI calls
...
binding_exists()     { scan_zone_bindings | grep -F -q "${1}|${2}|"; }
binding_get_ruleset() { scan_zone_bindings | grep -F "${1}|${2}|" | head -n 1 | awk -F'|' '{print $3}'; }
...
scan_rip_passive_interfaces(){ grep_cfg "set protocols rip passive-interface interface " | awk '{print $6}' | sort -u | while read -r x; do strip_quotes "$x"; done; }
...
get_iface_description() {
  ...
  grep_cfg "$pattern" | awk '{$1=$2=$3=$4=$5=""; print}' | sed 's/^[[:space:]]*//' | head -n 1 | while read -r x; do strip_quotes "$x"; done
}
...
# ============================================================
# COMPACT SUMMARIES  (only show what's relevant per submenu)
# ============================================================
_iface_summary() {
...
# run_cmd_to_tty — FIX-9: intentional unquoted $cmd (VyOS 'run' needs word-split args)
run_cmd_to_tty() {
  local cmd="$1"
  tprint ""; tprint ">> $cmd"
  tprint "--------------------------------------------------------"
...
# ============================================================
# INTERFACE MENU  (unified — replaces both old iface_menu + iface_expanded_menu)
# ============================================================
_iface_choose() {
...
iface_op_set_ip() {
  ...
  tprint "To remove an IP use: Delete IP address."
  tprint ""
...
zone_list_bindings() {
  ...
  tprint ""; tprint "Zone bindings (TO <- FROM = RULESET):"
...
zone_add_binding_safe() {
  ...
  if binding_exists "$to" "$from"; then
    tprint "ERROR: Binding $to <- $from already exists (ruleset: $(binding_get_ruleset "$to" "$from"))."
    tprint "Use Update to change it."; pause; return 0
  fi
...
zone_bindings_menu() {
  while true; do
    ...
    zone_list_bindings 2>/dev/null || true
...
# ============================================================
# ZONE MANAGEMENT (create/delete zones, assign interfaces,
#                  set default-action, intra-zone policy)
# ============================================================
scan_zone_ifaces() {
  # Returns lines of "zone|iface"
...
zone_list_full() {
  ...
  for z in "${zones[@]}"; do
    ...
    ifaces="$(scan_zone_ifaces | grep -F "$z|" | cut -d'|' -f2 | tr '\n' ' ' || echo -)"
...
zone_create_safe() {
  # VyOS BUG T7112 WORKAROUND:
  # Setting default-action before any interface is assigned (or before
  # local-zone keyword) triggers KeyError: member in firewall.py.
  # Fix: always set interface(s) OR local-zone BEFORE default-action
  # in the same cfg_begin/cfg_apply transaction.

  local zones=() zname zone_type da ifaces=() all_ifaces=() yn
  load_array zones scan_fw_zones
  tprint ""; tprint "Existing zones: ${zones[*]:-(none)}"; tprint ""
  tprint "Two zone types:"
  tprint "  normal     = group of interfaces (LAN, WAN, DMZ ...)"
  tprint "  local-zone = the router itself (traffic to/from VyOS)"
  tprint ""

  zname="$(ask "New zone name (e.g. LAN, WAN, DMZ, LOCAL)" "")"
  [ -z "$zname" ] && return 0
  ...
  select_from_list_default "Default action for unmatched traffic into $zname" "drop" "drop" "accept" "reject" || return 0
  da="$SELECTED"

  if [ "$zone_type" = "normal" ]; then
    tprint ""
    tprint "REQUIRED: Assign at least one interface now (T7112 bug workaround)."
    tprint "default-action cannot be committed on an empty zone — interfaces must"
    tprint "exist in the same transaction."
    tprint ""
    load_array all_ifaces scan_all_ifaces
    tprint "Available interfaces: ${all_ifaces[*]:-(none)}"
    tprint ""
    while true; do
      local iface; iface="$(ask "Interface to assign (blank to stop)" "")"
      [ -z "$iface" ] && break
      if ! is_safe_iface_name "$iface"; then
        tprint "ERROR: Invalid interface name."; continue
      fi
      if is_in_list "$iface" "${ifaces[@]}"; then
        tprint "Already added."; continue
      fi
      local other_zone
      other_zone="$(scan_zone_ifaces | grep -F "|$iface" | cut -d'|' -f1 | head -n 1 || true)"
      if [ -n "$other_zone" ]; then
        tprint "WARNING: $iface is already in zone '$other_zone'."
        local cont; cont="$(choose_yes_no "Add anyway?" "n" || echo "n")"
        [ "$cont" != "y" ] && continue
      fi
      ifaces+=("$iface")
      tprint "  Added: $iface"
    ...
    if [ "${#ifaces[@]}" -eq 0 ]; then
      tprint ""
      tprint "ERROR: At least one interface is required for a normal zone."
      tprint "Cannot create an empty zone with a default-action (VyOS bug T7112)."
      pause; return 0
    fi
  fi
  ...
  if [ "$zone_type" = "local" ]; then
    tprint "SUMMARY: create local-zone '$zname'  default-action=$da"
  else
    tprint "SUMMARY: create zone '$zname'  default-action=$da  interfaces=${ifaces[*]}"
  fi
  yn="$(choose_yes_no "Proceed?" "y" || echo "n")"
  [ "$yn" != "y" ] && { tprint "Canceled."; pause; return 0; }
  ...
  cfg_begin || return 0
  if [ "$zone_type" = "local" ]; then
    # local-zone keyword first, then default-action (no interface member)
    cfg_set firewall zone "$zname" local-zone
    cfg_set firewall zone "$zname" default-action "$da"
  else
    # Interfaces FIRST in the transaction, default-action after (T7112 fix)
    local i; for i in "${ifaces[@]}"; do
      cfg_set firewall zone "$zname" interface "$i"
    done
    cfg_set firewall zone "$zname" default-action "$da"
  fi
  cfg_apply
...
zone_delete_safe() {
  ...
  # Warn about any bindings referencing this zone
...
zone_assign_interface() {
  ...
  # Show current interfaces on this zone
  cur_ifaces=()
...
zone_remove_interface() {
  ...
  zname="$(zone_choose_existing)" || return 0
...
zone_set_default_action() {
  # T7112 guard: only set default-action if zone has members or is local-zone
  ...
  # Block if zone has no members and is not a local-zone (T7112 workaround)
...
zone_set_intrazone_action() {
  # intra-zone-filtering: allow or drop traffic within the same zone
...
zone_management_menu() {
  ...
  while true; do
    ...
    tprint "2) Create zone (safe)"
    tprint "3) Delete zone"
    tprint "4) Assign interface to zone"
    tprint "5) Remove interface from zone"
    tprint "6) Set default-action"
    tprint "7) Set intra-zone action"
    tprint "8) Zone bindings (TO/FROM/ruleset)"
    tprint "9) Back"
    local c; tread c "Select: " || continue
    case "$c" in
      1) zone_list_full ;;
      2) zone_create_safe ;;
      3) zone_delete_safe ;;
      4) zone_assign_interface ;;
      5) zone_remove_interface ;;
      6) zone_set_default_action ;;
      7) zone_set_intrazone_action ;;
      8) zone_bindings_menu ;;
      9) return 0 ;;
      *) tprint "Invalid." ;;
    esac
  done
...
nat_update_single_field() {
  ...
  case "$field" in
    ...
    "source address"|"destination address")
      ...
      cfg_begin || return 0
      local nside="${field%% *}"
      [ -z "$val" ] && cfg_delete nat "$type" rule "$n" "$nside" address || cfg_set nat "$type" rule "$n" "$nside" address "$val"
      cfg_apply ;;
    "source port"|"destination port"|"translation port")
      ...
      cfg_begin || return 0
      local np="${field%% *}"
      [ -z "$val" ] && cfg_delete nat "$type" rule "$n" "$np" port || cfg_set nat "$type" rule "$n" "$np" port "$val"
      cfg_apply ;;
    "translation address")
      tprint "Leave blank to DELETE. For SNAT: masquerade is valid."
      val="$(ask "Translation address" "")"
      [ -n "$val" ] && [ "$val" != "masquerade" ] && ! is_valid_ipv4 "$val" && ! is_valid_cidr4 "$val" && { tprint "ERROR."; pause; return 0; }
      cfg_begin || return 0
      [ -z "$val" ] && cfg_delete nat "$type" rule "$n" translation address || cfg_set nat "$type" rule "$n" translation address "$val"
      cfg_apply ;;
    "inbound-interface name"|"outbound-interface name")
      ...
      cfg_begin || return 0
      local ifdir="${field%%-*}"
      [ -z "$val" ] && cfg_delete nat "$type" rule "$n" "${ifdir}-interface" name || cfg_set nat "$type" rule "$n" "${ifdir}-interface" name "$val"
      cfg_apply ;;
    *) tprint "Invalid."; pause ;;
  esac
}
...
rip_add_passive_interface_safe() {
  ...
  select_from_list "Select interface or 'default'" "${choices[@]}" && iface="$SELECTED" || iface="$(ask "Interface or 'default'" "")"
  [ -z "$iface" ] && return 0
  ...
  [ "$yn" != "y" ] && { tprint "Canceled."; pause; return 0; }
  cfg_begin || return 0; cfg_set protocols rip passive-interface interface "$iface"; cfg_apply
}
...
rip_delete_passive_interface_existing() {
  ...
  [ "$yn" != "y" ] && { tprint "Canceled."; pause; return 0; }
  cfg_begin || return 0; cfg_delete protocols rip passive-interface interface "$target"; cfg_apply
}
...
static_route_delete() {
  ...
  # If multiple next-hops, offer to delete just one or the whole prefix
...
dhcp_add_pool_safe() {
  ...
  cfg_begin || return 0
  cfg_set service dhcp-server shared-network-name "$name" subnet "$subnet" \
    range 0 start "$range_start"
  cfg_set service dhcp-server shared-network-name "$name" subnet "$subnet" \
    range 0 stop "$range_stop"
  [ -n "$gateway" ] && cfg_set service dhcp-server shared-network-name "$name" \
    subnet "$subnet" option default-router "$gateway"
  [ -n "$dns" ] && cfg_set service dhcp-server shared-network-name "$name" \
    subnet "$subnet" option name-server "$dns"
  [ -n "$lease" ] && cfg_set service dhcp-server shared-network-name "$name" \
...
dhcp_add_static_mapping() {
  ...
  cfg_begin || return 0
  cfg_set service dhcp-server shared-network-name "$pool" subnet "$subnet" \
    static-mapping "$name" mac "$mac"
  cfg_set service dhcp-server shared-network-name "$pool" subnet "$subnet" \
    static-mapping "$name" ip-address "$ip"
  cfg_apply
}
...
ssh_is_enabled() {
  # SSH is enabled if any 'set service ssh' line exists
...
raw_mode() {
  ...
  # FIX-9: intentional word-splitting — 'run' needs separate args
  # shellcheck disable=SC2086
  set -- $cmd
...
main_menu() {
  ...
  while true; do
    # Refresh cache once per main menu loop so summary is always current
    cfg_cache_refresh 2>/dev/null || true

    tprint ""
    tprint "======================================"
    tprint " VyOS Dynamic Menu  (v2)"
    tprint "======================================"
