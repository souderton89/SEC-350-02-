...
zone_create_safe() {
  local zones=() name da ifaces=() all_ifaces=() yn
  load_array zones scan_fw_zones
  tprint ""; tprint "Existing zones: ${zones[*]:-(none)}"; tprint ""
  local zname; zname="$(ask "New zone name (e.g. LAN, WAN, DMZ)" "")"
  [ -z "$zname" ] && return 0
  ...
  # Interface assignment (optional — can be done later)
  load_array all_ifaces scan_all_ifaces
  tprint ""; tprint "Assign interfaces to $zname now (optional — press Enter to skip)."
  tprint "Available: ${all_ifaces[*]:-(none)}"
  while true; do
  ...
  tprint ""
  tprint "SUMMARY: create zone $zname  default-action=$da  interfaces=${ifaces[*]:-(none to assign now)}"
  yn="$(choose_yes_no "Proceed?" "y" || echo "n")"
  [ "$yn" != "y" ] && { tprint "Canceled."; pause; return 0; }
