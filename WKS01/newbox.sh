...
menu() {
  ...
  echo "5) Install git + clone/pull repo + create host folder (if missing -> commit+push) (existing user)"
  echo "6) Set authorized_keys from repo (existing user)"
  echo "7) Configure repo URL + public key path defaults"
  echo "8) Exit"
  echo
  ...
  case "$choice" in
    ...
    4) configure_network ;;
    5)
      read -r -p "Enter EXISTING username to configure repo for: " u
      [[ -n "$u" ]] || { echo "Username cannot be blank."; return; }
      if ! user_exists "$u"; then
        echo "ERROR: User '$u' does not exist."
        return
      fi
      # CHANGED: option 5 no longer asks for public key / authorized_keys
      clone_or_update_repo_for_user "$u"
      ;;
    6)
      read -r -p "Enter EXISTING username to set authorized_keys for: " u
      [[ -n "$u" ]] || { echo "Username cannot be blank."; return; }
      if ! user_exists "$u"; then
        echo "ERROR: User '$u' does not exist."
        return
      fi
      # NEW: dedicated menu option for public key prompt + authorized_keys setup
      setup_authorized_keys_from_repo "$u"
      ;;
    7) configure_repo_settings ;;
